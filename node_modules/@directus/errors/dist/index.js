...
const messageConstructor$18 = ({ collection, field }) => `Field "${field}" in collection "${collection}" contains null values.`;
const ContainsNullValuesError = createError(ErrorCode.ContainsNullValues, messageConstructor$18, 400);
...
const messageConstructor$17 = (extensions) => {
...
const EmailLimitExceededError = createError(ErrorCode.EmailLimitExceeded, messageConstructor$17, 429);

//#endregion
//#region src/errors/forbidden.ts
const messageConstructor$16 = (ext) => {
	if (ext?.reason) return ext.reason;
	return `You don't have permission to access this.`;
};
const ForbiddenError = createError(ErrorCode.Forbidden, messageConstructor$16, 403);

//#endregion
//#region src/errors/hit-rate-limit.ts
const messageConstructor$15 = (extensions) => {
	return `Too many requests, retry after ${ms(extensions.reset.getTime() - Date.now())}.`;
};
const HitRateLimitError = createError(ErrorCode.RequestsExceeded, messageConstructor$15, 429);
...
const messageConstructor$14 = ({ collection, field, value }) => {
...
const InvalidForeignKeyError = createError(ErrorCode.InvalidForeignKey, messageConstructor$14, 400);
...
const messageConstructor$13 = ({ reason }) => `Invalid payload. ${reason}.`;
const InvalidPayloadError = createError(ErrorCode.InvalidPayload, messageConstructor$13, 400);

//#endregion
//#region src/errors/invalid-path-parameter.ts
const messageConstructor$12 = ({ reason }) => `Invalid path parameter. ${reason}.`;
const InvalidPathParameterError = createError(ErrorCode.InvalidPathParameter, messageConstructor$12, 400);
...
//#endregion
export { ContainsNullValuesError, ContentTooLargeError, EmailLimitExceededError, ErrorCode, ForbiddenError, HitRateLimitError, IllegalAssetTransformationError, InternalServerError, InvalidCredentialsError, InvalidForeignKeyError, InvalidIpError, InvalidOtpError, InvalidPathParameterError, InvalidPayloadError, InvalidProviderConfigError, InvalidProviderError, InvalidQueryError, InvalidTokenError, LimitExceededError, MethodNotAllowedError, NotNullViolationError, OutOfDateError, RangeNotSatisfiableError, RecordNotUniqueError, RouteNotFoundError, ServiceUnavailableError, TokenExpiredError, UnexpectedResponseError, UnprocessableContentError, UnsupportedMediaTypeError, UserSuspendedError, ValueOutOfRangeError, ValueTooLongError, createError, isDirectusError };